...
public:
	...
	virtual	int	Init()
	{
		...
		SpiWrite(	0xCF, 0x00, 0x81, 0X30 );
		SpiWrite(	0xED, 0x64, 0x03, 0X12, 0X81 );
		SpiWrite(	0xE8, 0x85, 0x10, 0x7A );
		SpiWrite(	0xCB, 0x39, 0x2C, 0x00, 0x34, 0x02 );
		
		SpiWrite(	0xF7, 0x20 );
		SpiWrite(	0xEA, 0x00, 0x00 );
		
		SpiWrite(	0xC0, 0x21 );		// Power control, VRH[5:0]
		...
		SpiWrite(	0xF2, 0x00 );		// 3Gamma Function Disable
		SpiWrite(	0x26, 0x01 );		// Gamma curve selected
		SpiWrite(	0xE0, 0x0F, 0x23, 0x1F, 0x0B, 0x0E, 0x08, 0x4B, 0XA8, 0x3B, 0x0A, 0x14, 0x06, 0x10, 0x09, 0x00);	 //Set Gamma
		SpiWrite(	0XE1, 0x00, 0x1C, 0x20, 0x04, 0x10, 0x08, 0x34, 0x47, 0x44, 0x05, 0x0B, 0x09, 0x2F, 0x36, 0x0F ); //Set Gamma
...
public:
	...
	virtual	int	Init()
	{
		...
		SpiWrite(	0xF2, 0x00 );   	// 3Gamma Function Disable 
		SpiWrite(	0x26, 0x01 );   	// Gamma curve selected 
		SpiWrite(	0xE0, 0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08, 0x4E, 0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00);  //Set Gamma 
		SpiWrite(	0xE1, 0x00, 0x0E, 0x14, 0x03, 0x11, 0x07, 0x31, 0xC1, 0x48, 0x08, 0x0F, 0x0C, 0x31, 0x36, 0x0F);    //Set Gamma 
